...
export default function VotePage() {
  const [step, setStep] = useState('auth'); // auth | dup | pick | done
  ...
  const vote = async () => {
    ...
    setLoading(true);
    try {
      await submitVote(eid.trim(), nm.trim(), sel);
      setStep('done');
    } catch (e) {
      setErr(e.message);
    } finally {
  ...
  return (
    <PageShell accent="#6366f1" icon="🗳️" title="본선 인기투표" sub="가장 인상 깊은 팀에 투표하세요">
      {step === 'auth' && (
        <div className="flex justify-center min-h-[50vh] items-center">
          <GlassBox glow className="w-full max-w-md text-center">
      ...
      {step === 'dup' && (
        <div className="text-center py-9 animate-up">
          ...
          <p style={{ color: 'rgba(255,255,255,0.5)' }}>
            <strong style={{ color: '#a855f7' }}>{dupTeam?.name}</strong>에 투표하셨습니다
          </p>
        </div>
      )}
      ...
      {step === 'pick' && (
        <div className="animate-up">
          <p className="text-center text-xs mb-3.5" style={{ color: 'rgba(255,255,255,0.45)' }}>
            <strong style={{ color: '#6366f1' }}>{nm.trim()}</strong>님, 1팀을 선택하세요
          </p>
          <div className="flex flex-col gap-2.5">
      ...
      {step === 'done' && (
        <div className="text-center py-9 animate-up">
          ...
          <h2 className="text-2xl font-extrabold my-2.5" style={{ background: 'linear-gradient(to right,#6366f1,#a855f7)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            투표 완료!
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.5)' }}>
            <strong>{nm.trim()}</strong>님 → <strong style={{ color: '#a855f7' }}>{teams.find(t => t.id === sel)?.name}</strong>
          </p>
          <p className="text-xs mt-5" style={{ color: 'rgba(255,255,255,0.25)' }}>🏆 결과는 시상식에서 공개됩니다</p>
        </div>
      )}
    </PageShell>
  );
